...
$joursOuvresParSemaine = (int)$month['jours_ouvres_par_semaine'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Jours ouvrés
    $jours = isset($_POST['jours']) ? (int)$_POST['jours'] : $joursOuvresParSemaine;
    if (!in_array($jours, [5, 6, 7], true)) {
        $errors[] = 'Nombre de jours ouvrés invalide.';
    }
    // Jours cochés comme fériés
    $selected = $_POST['holidays'] ?? [];

    // Construire overrides pour jours ouvrés
    $newOverrides = [];
    if (!$isRefresh) {
    ...
    if (empty($errors)) {
        ...
        $joursOuvresParSemaine = $jours;
    }
}

$pageTitle = 'Éditer ' . htmlspecialchars($monthFormatter->format(new DateTimeImmutable("{$year}-{$mon}-01")), ENT_QUOTES);
...
<style>
    ...
    .holidays-table td.checkbox-cell {
        text-align: center;
        width: 4em;
    }
</style>
<div class="container content">
    ...
    <?php elseif ($success): ?>
        <div class="errors" style="background:#d4edda;color:#155724;border:1px solid #c3e6cb;"><?= htmlspecialchars($success, ENT_QUOTES) ?></div>
    <?php endif; ?>
    <form method="post" class="form-container">
        <label for="jours">Jours ouvrés par semaine</label>
        <select id="jours" name="jours">
            <?php foreach ([5, 6, 7] as $j): ?>
                <option value="<?= $j ?>" <?= $joursOuvresParSemaine == $j ? 'selected' : '' ?>><?= $j ?> jours</option>
            <?php endforeach; ?>
        </select>
        <button type="submit" name="refresh" class="btn" style="margin:1em 0;">Afficher les jours</button>
        <h3>Jours fériés</h3>
        <small>Cochez pour marquer en férié</small>
        <table class="holidays-table">
            ...
            <tbody>
                <?php for ($d = 1; $d <= $daysInMonth; $d++):
                    $date = sprintf('%04d-%02d-%02d', $year, $mon, $d);
                    $weekday = (int)(new DateTimeImmutable($date))->format('N');
                    if ($weekday > $joursOuvresParSemaine) continue;
                    $isFixed = in_array(substr($date, 5), $fixedHolidays, true);
                    $checked = $overrides[$date] ?? $isFixed;
                    $weekdayFr = ucfirst($weekdayFormatter->format(new DateTimeImmutable($date)));
                ?>
                    <tr>
                        <td><?= $d ?></td>
                        <td><?= htmlspecialchars($weekdayFr, ENT_QUOTES) ?><?php if ($isFixed): ?> <em>(fixe)</em><?php endif; ?></td>
                        <td class="checkbox-cell">
                            <input type="checkbox" name="holidays[]" value="<?= $date ?>" <?= $checked ? 'checked' : '' ?>>
                        </td>
                    </tr>
                <?php endfor; ?>
            </tbody>
        ...
        <a href="chef_mois.php" class="btn btn-secondary">Retour</a>
    </form>
</div>
