...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['delete_id'])) {
        ...
        // autoriser minutes négatives ou positives, mais pas zéro
        if ($minutes === 0) {
            $errors[] = 'Le nombre de minutes doit être différent de zéro.';
        }
...
$listStmt = $pdo->prepare(
    "SELECT id, DATE(date_saisie) AS d, minutes, created_at
      FROM heures_supplementaires
     WHERE user_id = ?
...
<div class="container content">
    ...
    <form method="get" class="form-container mb-4">
        ...
        <select name="month" id="month" onchange="this.form.submit()">
            <?php foreach ($monthRows as $ym): $dt = new DateTimeImmutable("$ym-01"); ?>
                <option value="<?= $ym ?>" <?= $ym === $selectedMonth ? 'selected' : '' ?>>
                    <?= htmlspecialchars(ucfirst($fmtMonthYear->format($dt)), ENT_QUOTES) ?></option>
            <?php endforeach; ?>
        </select>
    </form>
    ...
    <section class="form-container mb-4">
        ...
        <form method="post">
            ...
            <label for="minutes">Minutes (positif ou négatif)</label>
            <input type="number" id="minutes" name="minutes" step="1" required
                value="<?= htmlspecialchars($_POST['minutes'] ?? '', ENT_QUOTES) ?>">
    ...
    <section>
        ...
        <?php else: ?>
            <table class="planning-all">
                ...
                <tbody>
                    <?php foreach ($entries as $row): ?>
                        <tr>
                            <td><?= htmlspecialchars($fmtDate->format(new DateTimeImmutable($row['d'])), ENT_QUOTES) ?></td>
                            <td><?= htmlspecialchars($row['minutes'], ENT_QUOTES) ?></td>
                            <td><?= htmlspecialchars((new DateTimeImmutable($row['created_at']))->format('d/m/Y H:i'), ENT_QUOTES) ?></td>
                            <td>
                                <form method="post" style="display:inline">
                                    <input type="hidden" name="edit_id" value="<?= $row['id'] ?>">
                                    <input type="hidden" name="date" value="<?= $row['d'] ?>">
            ...
            <p class="mt-3">
                <strong>Total cumulé jusqu’à <?= htmlspecialchars(ucfirst($fmtMonthYear->format(new DateTimeImmutable("$selectedMonth-01"))), ENT_QUOTES) ?> :</strong>
